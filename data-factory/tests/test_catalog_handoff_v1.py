...
def compile_fixture(output: Path, **overrides: object) -> Path:
    ...
    values = {"metadata": metadata, "archive_artifact": archive, "extracted_artifact": extracted, "source_records": records, "composition_values": compositions, "food_concepts": concepts, "food_names": names, "source_food_mappings": mappings, "backend_baseline": "test-baseline"}
...
class CatalogHandoffV1Tests(unittest.TestCase):
    ...
    def test_compilation_is_byte_deterministic_and_input_order_independent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            metadata, archive, extracted, records, compositions, concepts, names, mappings = fixture_inputs()
            second = compile_catalog_handoff_v1(Path(directory) / "second", metadata=metadata, archive_artifact=archive, extracted_artifact=extracted, source_records=list(reversed(records)), composition_values=list(reversed(compositions)), food_concepts=list(reversed(concepts)), food_names=list(reversed(names)), source_food_mappings=list(reversed(mappings)), backend_baseline="test-baseline")
