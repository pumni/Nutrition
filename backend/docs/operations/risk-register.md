...
| Raw meal text leaks through telemetry or provider diagnostics | Medium | High | HTTP traces exclude body; static scan passes; hosted telemetry schema has no content fields; mock request privacy test; runtime structured log-capture sentinel test | Approve provider retention/privacy settings | Security owner |
...
| Provider outage exhausts API capacity | Medium | High | 5 s per-attempt timeout, one retry maximum, bounded response stream, per-provider/model no-queue bulkhead, provider/model circuit breaker | Tune concurrency with staging latency/error telemetry and provider quota | Backend owner |
